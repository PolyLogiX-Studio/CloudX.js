...
import { WorldStartupParametersJSON, WorldStartupParameters } from './WorldStartupParameters';
import { SessionJoinParameters, SessionJoinParametersJSON } from './SessionJoinParameters';
import { InputConfig, InputConfigJSON } from './InputConfig';
...
export class NeosConfig {
   ...
   */
  public Schema: string | "https://github.com/Neos-Metaverse/JSONSchemas/blob/main/schemas/NeosConfig.schema.json";
  /**
  ...
  constructor($b: NeosConfigJSON = {} as NeosConfigJSON) {
    this.Schema = $b["$schema"] ?? "https://github.com/Neos-Metaverse/JSONSchemas/blob/main/schemas/NeosConfig.schema.json"
    this.UnsafeModeWhitelist = $b.unsafeModeWhiteList instanceof List
      ? $b.unsafeModeWhiteList
      : List.ToList($b.unsafeModeWhiteList)
    this.Inputs = $b.inputs instanceof InputConfig ? $b.inputs : new InputConfig($b.inputs);
    this.UniverseId = $b.universeId
    this.AutoStartWorlds = $b.autoStartWorlds instanceof List
      ? $b.autoStartWorlds
      : List.ToListAs($b.autoStartWorlds, WorldStartupParameters);
    this.AutoJoinSessions = $b.autoJoinSessions = $b.autoJoinSessions instanceof List
      ? $b.autoJoinSessions
      : List.ToListAs($b.autoJoinSessions, SessionJoinParameters)
    this.NoUI = $b.noUI
    this.DisableDesktop = $b.disableDesktop
    this.PathWhitelist = $b.pathWhitelist instanceof List
      ? $b.pathWhitelist
      : List.ToList($b.pathWhitelist)
  }
}
export interface NeosConfigJSON {
  $schema: string | "https://github.com/Neos-Metaverse/JSONSchemas/blob/main/schemas/NeosConfig.schema.json"
  unsafeModeWhiteList: string[]
  inputs: InputConfigJSON
  universeId: string
  autoStartWorlds: WorldStartupParametersJSON[]
  autoJoinSessions: SessionJoinParametersJSON[]
  noUI: boolean
