...
import {
	CloudVariableProxy,
	CloudVariableIdentity,
	VariableReadBatchQuery,
	VariableWriteBatchQuery,
	CloudXInterface,
} from "../core/";
import {
	ACloudVariableEventHandler,
	CloudVariable,
	VariableReadRequest,
} from "../cloud";
...
export class CloudVariableManager {
	private _variableProxies = new Dictionary<
		CloudVariableIdentity,
		CloudVariableProxy
	>();
	...
	public ReadVariable(
		ownerId: string,
		path: string
	): Promise<VariableReadResult<CloudVariable, CloudVariableDefinition>> {
	...
	public Update(): void {
		try {
			if (this._changedVariables == null) return;
			const hashSet = pool;
			for (const changedVariable of this._changedVariables) {
				if (changedVariable.WriteToCloud()) hashSet.add(changedVariable);
			}
			for (const cloudVariableProxy of hashSet)
	...
	public async SignOut(): Promise<void> {
		const changedVariables = this._changedVariables;
		this._changedVariables = null as any;

		const taskList = new List<Promise<any>>();
		for (const cloudVariableProxy of changedVariables)
			taskList.Add(cloudVariableProxy.ForceWriteToCloud());
		await Promise.all(taskList);
	}

	public RequestProxy(ownerId: string, path: string): CloudVariableProxy {
		const variableIdentity = new CloudVariableIdentity(ownerId, path);
		const cloudVariableProxy = new Out<CloudVariableProxy>();
		if (
			!this._variableProxies.TryGetValue(variableIdentity, cloudVariableProxy)
		) {
	...
	public RegisterListener(
		ownerId: string,
		path: string,
		onChanged: ACloudVariableEventHandler
	): CloudVariableProxy {
		if (!CloudVariableHelper.IsValidPath(path))
			throw new Error("Invalid path: " + path);
		const proxy = this.RequestProxy(ownerId, path);
		proxy.Register(onChanged);
		if (
			proxy.State != CloudVariableState.Uninitialized &&
			proxy.State != CloudVariableState.Invalid
		)
			onChanged(proxy);
		return proxy;
	}
